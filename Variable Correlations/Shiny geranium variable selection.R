...
data(wrld_simpl)
shiny.geranium.train <- Geranium_data_train
SG.lat.lon.train <- shiny.geranium.train[ , c("decimallon", "decimallat")]
...
bioclim.extract.train <- raster::extract(bioclim.crop, SG.lat.lon.train, method = 'simple')
landuse.extract.train <- raster::extract(landuse.crop, SG.lat.lon.train, method = 'simple')
HII.extract.train <- raster::extract(HII.crop, SG.lat.lon.train, method = 'simple')

# combine dataframes (coordinates + bioclim values)extra
SG.variables.train <- cbind(SG.lat.lon.train, bioclim.extract.train, landuse.extract.train, HII.extract.train)

plot(SG.variables.train)

library(tidyverse)
# remove NA values 
SG.variables.train <- SG.variables.train%>%
...
SG.cor.data.train <- subset(SG.variables.train, select = -c(decimallat, decimallon))

##### Correlation tests ##### -----
# Pearson's correlation test
SG.cor.train <- cor(SG.cor.data.train, method = "pearson", use = "pairwise.complete.obs")
corrplot(cor(SG.cor.train, method = "pearson"))
...
# VIF
vif <- vifstep(SG.cor.data.train, th = 10) # threshold value of 10 
vif
...
# extract predictor values at species record points *remember this is only the training data
shiny.geranium.test <- Geranium_data_test
SG.lat.lon.test <- shiny.geranium.test[ , c("decimallon", "decimallat")]
SG.occ.test <- shiny.geranium.test[ , c("decimallon", "decimallat")] # for later

# extract bioclim variables for each species record, creating a dataframe
bioclim.extract.test <- raster::extract(bioclim.crop, SG.lat.lon.test, method = 'simple')
landuse.extract.test <- raster::extract(landuse.crop, SG.lat.lon.test, method = 'simple')
HII.extract.test <- raster::extract(HII.crop, SG.lat.lon.test, method = 'simple')

# combine dataframes (coordinates + bioclim values)extra
SG.variables.test <- cbind(SG.lat.lon.test, bioclim.extract.test, landuse.extract.test, HII.extract.test)

plot(SG.variables.test)

# remove NA values 
SG.variables.test <- SG.variables.test%>%
  filter(!is.na(wc2.1_30s_bio_1))%>%
  filter(!is.na(wc2.1_30s_bio_2))%>%
  filter(!is.na(wc2.1_30s_bio_3))%>%
  filter(!is.na(wc2.1_30s_bio_4))%>%
...
# check
summary(SG.variables.test)
# SG.occ.train has 1575 obs but SG.env.vars now has 1550 (25 NAs removed) so we need to bring SG.occ.train down 
# to 1550 as well (25 obs don't have explanatory variables)

# create subset without longitude and latitude for correlation
SG.cor.data.test <- subset(SG.variables.test, select = -c(decimallat, decimallon))

##### Correlation tests ##### -----
# Pearson's correlation test
SG.cor.test <- cor(SG.cor.data.test, method = "pearson", use = "pairwise.complete.obs")
corrplot(cor(SG.cor.test, method = "pearson"))
...
# VIF
vif <- vifstep(SG.cor.data.test, th = 10) # threshold value of 10 
vif
