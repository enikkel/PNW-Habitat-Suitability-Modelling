...
data(wrld_simpl)
mouseear.hawkweed.train <- officinarum_data_train
MEH.lat.lon.train <- mouseear.hawkweed.train[ , c("decimallon", "decimallat")]
MEH.occ.train <- mouseear.hawkweed.train[ , c("decimallon", "decimallat")] # for later

# extract bioclim variables for each species record, creating a dataframe
bioclim.extract.train <- raster::extract(bioclim.crop, MEH.lat.lon.train, method = 'simple')
landuse.extract.train <- raster::extract(landuse.crop, MEH.lat.lon.train, method = 'simple')
HII.extract.train <- raster::extract(HII.crop, MEH.lat.lon.train, method = 'simple')

# combine dataframes (coordinates + bioclim values)extra
MEH.variables.train <- cbind(MEH.lat.lon.train, bioclim.extract.train, landuse.extract.train, HII.extract.train)

plot(MEH.variables.train)

# remove NA values 
MEH.variables.train <- MEH.variables.train%>%
...
MEH.cor.data.train <- subset(MEH.variables.train, select = -c(decimallat, decimallon))

##### Correlation tests ##### -----
# Pearson's correlation test
MEH.cor.train <- cor(MEH.cor.data.train, method = "pearson", use = "pairwise.complete.obs")
corrplot(cor(MEH.cor.train, method = "pearson"))
...
# VIF
vif <- vifstep(MEH.cor.data.train, th = 10) # threshold value of 10 
vif
...
# extract predictor values at species record points *remember this is only the training data
mouseear.hawkweed.test <- officinarum_data_test
MEH.lat.lon.test <- mouseear.hawkweed.test[ , c("decimallon", "decimallat")]
MEH.occ.test <- mouseear.hawkweed.test[ , c("decimallon", "decimallat")] # for later

# extract bioclim variables for each species record, creating a dataframe
bioclim.extract.test <- raster::extract(bioclim.crop, MEH.lat.lon.test, method = 'simple')
landuse.extract.test <- raster::extract(landuse.crop, MEH.lat.lon.test, method = 'simple')
HII.extract.test <- raster::extract(HII.crop, MEH.lat.lon.test, method = 'simple')

# combine dataframes (coordinates + bioclim values)extra
MEH.variables.test <- cbind(MEH.lat.lon.test, bioclim.extract.test, landuse.extract.test, HII.extract.test)

plot(MEH.variables.test)

# remove NA values 
MEH.variables.test <- MEH.variables.test%>%
  filter(!is.na(wc2.1_30s_bio_1))%>%
  filter(!is.na(wc2.1_30s_bio_2))%>%
  filter(!is.na(wc2.1_30s_bio_3))%>%
  filter(!is.na(wc2.1_30s_bio_4))%>%
...
# check
summary(MEH.variables.test)
# SG.occ.train has 1575 obs but SG.env.vars now has 1550 (25 NAs removed) so we need to bring SG.occ.train down 
# to 1550 as well (25 obs don't have explanatory variables)

# create subset without longitude and latitude for correlation
MEH.cor.data.test <- subset(MEH.variables.test, select = -c(decimallat, decimallon))

##### Correlation tests ##### -----
# Pearson's correlation test
MEH.cor.test <- cor(MEH.cor.data.test, method = "pearson", use = "pairwise.complete.obs")
corrplot(cor(MEH.cor.test, method = "pearson"))
...
# VIF
vif <- vifstep(MEH.cor.data.test, th = 10) # threshold value of 10 
vif
