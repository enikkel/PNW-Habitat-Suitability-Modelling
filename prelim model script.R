...
SG.occ$Geranium.lucidum <- 1 # if wanting to use all data, to split

myRespName <- 'Geranium.lucidum'
myResp <- as.numeric(SG.occ[,myRespName])
myRespXY <- SG.occ[,c("Longitude","Latitude")]
...
# the formatting ran correctly, but such high res doesn't work in this plot() function therefore:
# Follow these steps to extract values and then visualize the biomod formatting output
...
(pres.xy <- get_PAtab(geranium_data_4) %>% 
    filter(status == 1) %>%
    select(x, y))

## get the coordinates of absences ==> No absences here
(abs.xy <- get_PAtab(geranium_data_4) %>% 
    filter(status == 0) %>%
    select(x, y))

## get the coordinates of pseudo - absences
## all repetition of pseudo absences sampling merged 
(pa.all.xy <- get_PAtab(geranium_data_4) %>% 
    filter(is.na(status)) %>%
    select(x, y)) %>%
  distinct()
## pseudo absences sampling for the first repetition only 
(pa.1.xy <- get_PAtab(geranium_data_4) %>% 
    filter(is.na(status) & PA1 == TRUE) %>%
    select(x, y)) %>%
  distinct()

(pa.2.xy <- get_PAtab(geranium_data_4) %>% 
    filter(is.na(status) & PA2 == TRUE) %>%
    select(x, y)) %>%
  distinct()

(pa.3.xy <- get_PAtab(geranium_data_4) %>% 
    filter(is.na(status) & PA3 == TRUE) %>%
    select(x, y)) %>%
  distinct()

plot(get_mask(geranium_data_4)[['PA1']])
points(pres.xy, cex = 0.5, col = 'black')
points(pa.1.xy, cex = 0.5, col = 'red')
points(pa.2.xy, cex = 0.5, col = 'yellow')
points(pa.3.xy, cex = 0.5, col = 'orange')
...
Ger.4.ModelOut # to view

# get all model evaluations
Ger.4.ModelEval <- get_evaluations(Ger.4.ModelOut)
dimnames(Ger.4.ModelEval)

# print out KAPPA scores for models
Ger.4.ModelEval["KAPPA", "Testing.data",,,]
Ger.4.ModelEval["TSS", "Testing.data",,,]
Ger.4.ModelEval["ROC", "Testing.data",,,]

# relative importance of variables
get_variables_importance(Ger.4.ModelOut)

Ger.4.EM <- BIOMOD_EnsembleModeling(
  modeling.output = Ger.4.ModelOut,
  chosen.models = 'all',
  em.by='all',
  eval.metric = c('TSS'),
...
Ger.4.CurrentProj <- BIOMOD_Projection(
  modeling.output = Ger.4.ModelOut,
  new.env = myExpl,
  proj.name = 'current',
  selected.models = 'all',
...
plot(Ger.4.CurrentProj, str.grep = 'MARS')
plot(Ger.4.CurrentProj, str.grep = 'GLM')
plot(Ger.4.CurrentProj, str.grep = 'GBM')
plot(Ger.4.CurrentProj, str.grep = 'RF')

Ger.4.allcurrentproj <- get_predictions(Ger.4.CurrentProj)
Ger.4.allcurrentproj

# make ensemble forecasting
Ger.4.EF <- BIOMOD_EnsembleForecasting(EM.output = Ger.4.EM,
                                       projection.output = Ger.4.CurrentProj)
plot(Ger.4.EF)
...
# continuing from geranium_data_4
# resolution too high to see data points but can extract:
...
# presence-only evaluation methods? Boyce using BIOMOD_presenceonly
Ger.4.pres.only.eval <- BIOMOD_presenceonly(Ger.4.ModelOut, Ger.4.EM)
# need to change em.by = PA_dataset+repet in order to use BIOMOD_presenceonly
